...
def play_game(agent_model, opponent_class):
    ...
    while game.numMoves >= 0:
        if game.currentPlayer == agent_id:
            ...
            q_value = agent_model(obs_tensor)
...
def evaluate(agent_path, games=5):
    model = Scrabbler(input_dim=154, hidden_dims=[154, 77, 10])
    ...
    print(f"\n🎯 Win Rate: {agent_wins / games * 100:.2f}%")
...
if __name__ == "__main__":
    print("Evaluating DeepQAgent vs RandomOpponent...")
    evaluate("scrabble_deepq.pt", games=3)
