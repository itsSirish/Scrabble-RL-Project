...
class Board():
    ...
    def place_word(self, start_coordinate, word, direction):
        ...
        if any(index > len(self.board) for index in end_coordinate):
            raise IllegalMoveError("The length of word is out of bounds of the board")
    ...
    def find_best_moves(self, rack, direction, dictionary, tile_set):
        has_any_letters = any(
            square.letter for row in self.board for square in row
        )
        anchors_used = []
        moves = []
        ...
        def is_anchor(coord):
            ...
            if not has_any_letters:
                # First move → allow anything
                return coord == (7, 7)
            # Must be adjacent to a tile (up, down, left, right)
